...
.device{
  ...
  position:relative;flex:0 0 auto;height:var(--device-h);width:calc(var(--device-h) * 0.4615);
...
.login-body{flex:1;display:flex;flex-direction:column;justify-content:center;
  padding:14px 30px 40px;overflow-y:auto}
.login-body::-webkit-scrollbar{width:0}
.brand{text-align:center;margin-bottom:30px}
.brand img{width:152px;height:auto;display:block;margin:0 auto;
  filter:drop-shadow(0 10px 22px rgba(62,39,35,.22))}
.brand .tag{color:var(--goud);font-size:11.5px;letter-spacing:4px;text-transform:uppercase;margin-top:14px;font-weight:700}
.tabs{display:flex;background:#F0E8D8;border-radius:15px;padding:5px;margin-bottom:22px}
.tabs button{flex:1;border:none;background:none;padding:12px;border-radius:11px;
...
.btn{width:100%;margin-top:24px;padding:16px;border:none;border-radius:15px;cursor:pointer;
...
function sizeDevice(){var d=document.querySelector('.device');if(!d)return;var r=9/19.5;var mh=window.innerHeight*0.97,mw=window.innerWidth*0.97;var h=Math.min(mh,940),w=h*r;if(w>mw){w=mw;h=w/r;}d.style.height=Math.round(h)+'px';d.style.width=Math.round(w)+'px';}
...
function prodImg(p){
  ...
  s+='<ellipse cx="60" cy="106" rx="23" ry="4.5" fill="#000" opacity=".13"/>';
  /* verpakkingslichaam (staande wikkel) */
  s+='<rect x="42" y="13" width="36" height="95" rx="5" fill="url(#'+id+'f)" stroke="'+c.edge+'" stroke-width="1"/>';
  /* gecrimpte foil boven en onder */
  function crimp(y){var r='<rect x="42" y="'+y+'" width="36" height="7" fill="'+c.sq+'" opacity=".5"/>';
    for(var x=44;x<78;x+=3){r+='<line x1="'+x+'" y1="'+y+'" x2="'+x+'" y2="'+(y+7)+'" stroke="'+c.bg1+'" stroke-opacity=".20" stroke-width=".7"/>';} return r;}
  s+=crimp(13)+crimp(101);
  /* cacaopeul-embleem (merk) op de foil boven het etiket */
  s+='<g transform="translate(60,31)"><path d="M0,-7 C3.4,-4.4 3.4,4.4 0,7 C-3.4,4.4 -3.4,-4.4 0,-7 Z" fill="'+c.pod+'"/><path d="M0,-6 L0,6 M-1.7,-5 L-1.7,5 M1.7,-5 L1.7,5" stroke="#fff" stroke-opacity=".35" stroke-width=".5" fill="none"/></g>';
  /* cremekleurig etiket */
  s+='<rect x="37" y="44" width="46" height="42" rx="4" fill="#F8F0E0" stroke="'+c.edge+'" stroke-width="1"/>';
  s+='<text x="60" y="53" text-anchor="middle" font-family="Segoe UI,Arial,sans-serif" font-size="3.6" letter-spacing="1.2" font-weight="700" fill="'+c.pod+'">CACAO CO.</text>';
  s+='<text x="60" y="66" text-anchor="middle" font-family="Georgia,serif" font-size="8" font-weight="700" fill="'+ink+'">'+(p.categorie||'').toUpperCase()+'</text>';
  s+='<line x1="49" y1="71" x2="71" y2="71" stroke="'+c.pod+'" stroke-width=".8"/>';
  s+='<text x="60" y="80.5" text-anchor="middle" font-family="Segoe UI,Arial,sans-serif" font-size="3.6" letter-spacing="1" fill="#8a7a6a">'+(PCT[p.code]||'FIJNE CHOCOLADE')+'</text>';
  /* smaakaccent op de onderste foil */
  var g=GARNISH[p.code];
  if(g&&g[0]==='dots'){ [[52,93,1.5],[60,97,1.4],[68,93,1.5],[58,99.5,1.2]].forEach(function(d){ s+='<circle cx="'+d[0]+'" cy="'+d[1]+'" r="'+d[2]+'" fill="'+g[1]+'"/>'; }); }
  else if(g&&g[0]==='leaf'){ s+='<path d="M60,90 C55,90.5 53,96 60,98.5 C61,94 64,91.5 60,90 Z" fill="#8aa55e"/>'; }
  else if(g&&g[0]==='nut'){ s+='<circle cx="60" cy="95" r="4" fill="#8a5a33" stroke="#6c4426" stroke-width=".7"/><path d="M60 91.5 L60 98.5" stroke="#6c4426" stroke-width=".5"/>'; }
  /* glans + lichtrand */
  s+='<rect x="42" y="13" width="36" height="95" rx="5" fill="url(#'+id+'sh)"/>';
  s+='<rect x="44.5" y="22" width="1.6" height="78" rx="1" fill="#fff" opacity=".10"/>';
  s+='</svg>';
  return s;
}
...
/* ============== HOME ============== */
function renderHome(){
  var s=STATE,h='',voornaam=(s.account.naam||'').split(' ')[0]||'klant';
  ...
  if(s.bijna_verlopen.length){
    h+='<h2 class="sectie">Bijna verlopen'+hIc("clock")+'</h2>';
    s.bijna_verlopen.forEach(function(p){
      var dg=p.dagen_resterend, tekst=dg<0?'Verlopen':(dg===0?'Verloopt vandaag':'Nog '+dg+' dag'+(dg===1?'':'en'));
  ...
  h+='<div class="row-between" style="margin:20px 4px 11px"><h2 class="sectie" style="margin:0">Mijn collectie</h2>'+
  ...
  }else{h+='<div class="kaart"><div class="leeg"><span class="e">'+ic("box")+'</span>Nog geen producten geregistreerd. Scan een reep om te beginnen.</div></div>';}
  ...
  }else{h+='<div class="kaart"><div class="leeg"><span class="e">'+ic("bag")+'</span>Nog geen bestellingen geplaatst.</div></div>';}
  h+='<div class="row-between" style="margin:20px 4px 11px"><h2 class="sectie" style="margin:0">Notificaties</h2>'+
     '<span class="muted" style="cursor:pointer;color:var(--goud)" onclick="openNotificaties()">Alles bekijken</span></div>';
  if(s.notifications.length){s.notifications.slice(0,3).forEach(function(n){h+=notifHTML(n);});}
  else{h+='<div class="kaart"><div class="leeg"><span class="e">'+ic("bell")+'</span>Geen notificaties.</div></div>';}
